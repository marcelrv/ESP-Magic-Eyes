...
- **Device name on the network**: `WifiManager` sets the WiFi hostname (NVS `hostname`, default `esp-magic-eyes`) before the first `WiFi.mode()`, so DHCP and router device lists show that name. On the first home-network connection it starts mDNS (`<hostname>.local`), advertising `_http._tcp` with the device name as instance name, and never stops it (the IDF responder handles reconnects). ArduinoOTA runs with its own mDNS disabled, because its `end()` calls `MDNS.end()`: the name used to disappear whenever network OTA stopped. `OtaManager` only adds and removes the `_arduino._tcp` advert (port 3232).
